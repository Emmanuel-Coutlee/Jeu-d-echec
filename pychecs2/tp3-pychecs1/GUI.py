...
class fenetre(Tk):
    ...
    def menu(self):
        self.menu_bar= Menu(bg = 'red')
        self.partie_menu = Menu(self.menu_bar, tearoff = 0)
        self.partie_menu.add_command(label= 'Nouvelle partie', command = self.quit)
        self.partie_menu.add_command(label= 'Ouvrir une partie', command = self.quit)
        self.partie_menu.add_command(label= 'Sauvegarder la partie', command = self.quit)
        self.partie_menu.add_command(label= 'Quitter', command = self.quit)


        self.affichage_menu = Menu(tearoff = 0)
        #################################
        self.couleur_button = Button(command = self.canvas_echiquier.changer_couleur_1(2,'red'))
        self.affichage_menu.add_command(label= 'Modifier', command = self.couleur_button)

        self.affichage_menu.add_command(label= 'résolution',)


        self.aide_menu = Menu(tearoff= 0)
        self.aide_menu.add_command(label= 'Règle de jeu')
        self.aide_menu.add_command(label= 'À propos')
